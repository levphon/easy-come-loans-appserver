...
###########init variable#############
JAVA_HOME="/app/soft/jdk/jdk-9.0"
SERVER_PORT="8885"
DUBBO_PORT=""
...
if [ -n "$BITS" ]; then
        if [ -n "$JMX_ARG" ];then
            JAVA_MEM_OPTS="-server $JMX_ARG -XX:PermSize=128m -Xss256k -XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70"
        else
            JAVA_MEM_OPTS=" -server -Xmx1g -Xms1g -Xmn256m -XX:PermSize=128m -Xss256k -XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 "
        fi
else
   ...
   while [ $COUNT -lt 1 ]; do 
        ...
        if [ -n "$RESULT" ];then
            if [ -n "$DUBBO_PORT" ]; then
                ...
                COUNT=`/bin/ss -lpn | grep "${DUBBO_PORT}.*$PIDS" | wc -l`
            else
                   if [ -n "$SERVER_PORT" ]; then
                       #COUNT=`netstat -ntl | grep -w $SERVER_PORT | wc -l`
                       COUNT=`/bin/ss -lpn | grep "${SERVER_PORT}.*$PIDS" | wc -l`
                   fi
            fi
