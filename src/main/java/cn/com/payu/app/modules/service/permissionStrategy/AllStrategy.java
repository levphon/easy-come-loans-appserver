...
public class AllStrategy extends PermissionStrategy {
    ...
    public List<MngUser> permissionUsersByDepartmentId(Long departmentId) {
        ...
        List<MngUser> userList = mngUserMapper.selectByDepartmentIds(subOrgIdList);
    ...
    public List<? extends TreeModel> orgTree(OrgTreeSearch search) {
        ...
        if (search.getHasUserLeaf() && CollectionUtils.isNotEmpty(departmentIdList)) {
            List<OrgModel> allUserList = mngUserMapper.selectUserOrgModels(new MngUserSearch().setDepartmentIds(departmentIdList).setUserIds(search.getUserIds()));
